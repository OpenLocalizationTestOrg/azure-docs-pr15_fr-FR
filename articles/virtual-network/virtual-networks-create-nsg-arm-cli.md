<properties 
   pageTitle="Comment créer des NSGs en mode processeur à l’aide de l’infrastructure du langage commun Azure | Microsoft Azure"
   description="Découvrez comment créer et déployer NSGs dans processeur à l’aide de l’infrastructure du langage commun Azure"
...
[AZURE.INCLUDE [azure-arm-classic-important-include](../../includes/azure-arm-classic-important-include.md)]Cet article décrit le modèle de déploiement du Gestionnaire de ressources. Vous pouvez également [créer NSGs dans le modèle de déploiement classique](virtual-networks-create-nsg-classic-cli.md).
...
3. Exécutez la commande **réseau azure nsg créer** pour créer un NSG.
    ...
    Paramètres :
    - **-g (ou--groupe de ressources)**. Nom du groupe de ressources où la NSG est créé. Pour notre scénario, *TestRG*.
...
4. Exécutez la commande **règle de nsg réseau azure créer** pour créer une règle qui autorise l’accès au port 3389 (RDP) à partir d’Internet.
    ...
    - **-c (ou--access)**. Niveau d’accès de la règle (autoriser ou refuser).
    - **-p (ou--protocol)**. Protocole (Tcp, Udp, ou *) pour la règle.
    - **-r (ou--direction)**. Sens de connexion (entrant ou sortant).
...
5. Exécutez la commande **règle de nsg réseau azure créer** pour créer une règle qui autorise l’accès au port 80 (HTTP) à partir d’Internet.
...
6. Exécutez la commande **définie vnet sous-réseau azure** pour lier la NSG au sous-réseau frontal.
...
4. Exécutez la commande **règle de nsg réseau azure créer** pour créer une règle qui autorise l’accès au port 1433 (SQL) à partir du sous-réseau frontal.
...
5. Exécutez la commande **règle de nsg réseau azure créer** pour créer une règle qui refuse l’accès à Internet à partir de.
...
6. Exécutez la commande **définie vnet sous-réseau azure** pour lier la NSG au sous-réseau principal.
