<properties 
   pageTitle="Déployer un ordinateur virtuel avec une adresse IP statique public à l’aide de l’infrastructure du langage commun Azure dans le Gestionnaire de ressources | Microsoft Azure"
   description="Découvrez comment déployer des machines virtuelles avec une adresse IP statique publique à l’aide de l’infrastructure du langage commun Azure dans le Gestionnaire de ressources"
...
# <a name="deploy-a-vm-with-a-static-public-ip-using-the-azure-cli"></a>Déployer un ordinateur virtuel avec une adresse IP statique public à l’aide de l’infrastructure du langage commun Azure
...
Vous pouvez télécharger le script bash complète utilisé [ici](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/03-Static-public-IP/virtual-network-deploy-static-pip-arm-cli.sh). Suivez les étapes ci-dessous pour modifier le script pour l’utiliser dans votre environnement.
...
4. Créez l’interface de réseau (carte réseau) de la machine virtuelle dans le sous-réseau créé ci-dessus, avec l’adresse IP publique. Notez le premier jeu de commandes sont utilisés pour récupérer l' **Id** du sous-réseau créé ci-dessus.
    ...
    >[AZURE.TIP] La première commande ci-dessus utilise [grep](http://tldp.org/LDP/Bash-Beginners-Guide/html/sect_04_02.html) et [manipulation de chaîne](http://tldp.org/LDP/abs/html/string-manipulation.html) (plus précisément, suppression sous-chaîne). 
