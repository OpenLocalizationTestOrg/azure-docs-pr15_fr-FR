<properties
   pageTitle="Exemple de cycle de vie d’une application basée sur le reste | Microsoft Azure"
   description="Un exemple de tissu de Service Microsoft Azure qui indique le cycle de vie d’application à l’aide de l’interface de Service tissu reste."
...
# <a name="rest-based-application-lifecycle-sample"></a>Exemple de cycle de vie d’une application basée sur le reste

Cet exemple illustre le cycle de vie application Service TISSU via des appels API REST. Pour plus d’informations sur le cycle de vie application tissu de Service, voir [leur cycle de vie tissu de Service](service-fabric-application-lifecycle.md).

Cet exemple effectue les opérations suivantes :

* Dispositions stocker de l’exemple **WordCount 1.0.0** du WordCount package d’application dans l’image.
* Affiche la liste des types d’applications, qui inclut WordCount 1.0.0.
* Crée l’application WordCount comme **TISSU : / WordCount**.
* Affiche la liste des applications, qui inclut TISSU : / WordCount version 1.0.0.
* Version de dispositions la 1.1.0 de l’exemple WordCount du **WordCountUpgrade** package d’application dans le magasin d’image.
* Affiche la liste des types d’applications, qui inclut WordCount 1.0.0 et **WordCount 1.1.0**.
* Met à jour l’application WordCount vers la version 1.1.0.
* Affiche la liste des applications, qui inclut WordCount version 1.1.0, mais n’inclut plus WordCount version 1.0.0.
* Supprime l’application WordCount.
* Affiche la liste des applications, qui n’est plus inclut TISSU : / WordCount.
* Version unprovisions la 1.1.0 de l’exemple WordCount.
* Affiche la liste des types d’applications, qui inclut WordCount 1.0.0, mais n’inclut plus WordCount 1.1.0.
* Version unprovisions la 1.0.0 de l’exemple WordCount.
* Affiche la liste des types d’applications, qui n’est plus inclut WordCount.


## <a name="prerequisites"></a>Conditions préalables

Cet exemple utilise l' [exemple WordCount](http://aka.ms/servicefabricsamples) (figurant dans les exemples de **Mise en route** ). L’exemple WordCount doit être généré en premier, puis ensuite deux packages d’application doivent être copiés dans le magasin d’image.

|Dossier|Description|
|------|-----------|
|WordCount|L’exemple WordCount d’application. Le fichier **ApplicationManifest.xml** contient **ApplicationTypeVersion = « 1.0.0 »**.|
|WordCountUpgrade|L’exemple WordCount d’application. Le fichier ApplicationManifest.xml doit être remplacé par **ApplicationTypeVersion = « 1.1.0 »** pour autoriser la mise à niveau de l’application se produise.|

Pour créer les packages d’application et les copier dans le magasin d’image, procédez comme suit :

1. Copiez **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** dans **C:\Temp\WordCount**. Cela crée le package d’application WordCount.
2. Copiez C:\Temp\WordCount dans **C:\Temp\WordCountUpgrade**. Cela crée le package **d’application WordCountUpgrade** .
3. Ouvrez **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** dans un éditeur de texte.
4. Dans l’élément **ApplicationManifest** , modifiez l’attribut **ApplicationTypeVersion** à **« 1.1.0 »**.  Cela met à jour le numéro de version de l’application.
5. Enregistrez le fichier ApplicationManifest.xml modifié.
6. Exécuter le script PowerShell suivant en tant qu’administrateur pour copier les applications dans le magasin d’image :
...
Une fois le script PowerShell, cette application est prête à être exécuté.

## <a name="example"></a>Exemple

L’exemple suivant illustre le cycle de vie application tissu de Service.
